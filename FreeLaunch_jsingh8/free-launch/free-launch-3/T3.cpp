...
class T3Handler : public MatchFinder::MatchCallback {
    private:
        Rewriter *rewriter;
        map <const FunctionDecl *, int> *decls;

    public :
        T3Handler(Rewriter *rewriter, map <const FunctionDecl *, int> *p_decls) : rewriter(rewriter), decls(p_decls) {}
        ...
        virtual void run(const MatchFinder::MatchResult &Result) {
		    ...
		    PrintingPolicy policy(lo);
                ...
                string unwrapArgs = 
                    "\n FL_T3_Postloop; \
                     \n char * _tmp_p = (char*)(FL_Args+ckernelSeqNum*FL_childKernelArgSz); \
                     \n int FL_childGridSize; \
                     \n memcpy((void*)&FL_childGridSize, (void*)_tmp_p, sizeof(int)); \
                     \n _tmp_p+=sizeof(int);";
                string childCode =
                    "\nfor(int kkk=0; kkk< FL_childGridSize; kkk++){ \n" + ref.str() + "\n";
                ...
                // Update Return with continue. TODO: But with replacing return.
                findAndReplace(childCode, "return;", "continue;");
                findAndReplace(childCode, "blockIdx.x", "kkk");
                ...
                start_replace.push_back("goto P;");
                ...
                rewriter->InsertTextAfter(parentKernel->getBody()->getLocEnd(),
                                          unwrapArgs + childCode + endFunctionCode);
            }
        }
};

class T3ASTConsumer : public ASTConsumer {
    public:
        T3ASTConsumer(Rewriter *rewriter, map <const FunctionDecl *, int> *decls) : FPrinter(rewriter, decls), APrinter(rewriter, decls) {
            Finder.addMatcher(
                functionDecl(
                    hasDescendant( 
                        cudaKernelCallExpr(
...
class T3FrontendAction : public ASTFrontendAction {
    public:
        T3FrontendAction() {}
        void EndSourceFileAction() override {
            RewriteBuffer &temp = rewriter.getEditBuffer(rewriter.getSourceMgr().getMainFileID());
            temp.InsertText(0, "#include \"freeLaunch_T3.h\"\n");
        ...
        std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI,
            ...
            return make_unique<T3ASTConsumer>(&rewriter, &decls);
        }
...
int main(int argc, const char **argv) {
    ...
    return Tool.run(newFrontendActionFactory<T3FrontendAction>().get());
}
