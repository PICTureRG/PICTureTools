...
class T2Handler : public MatchFinder::MatchCallback {
    private:
        Rewriter *rewriter;
        map <const FunctionDecl *, int> *decls;

    public :
        T2Handler(Rewriter *rewriter, map <const FunctionDecl *, int> *p_decls) : rewriter(rewriter), decls(p_decls) {}
        ...
        virtual void run(const MatchFinder::MatchResult &Result) {
		    ...
		    PrintingPolicy policy(lo);
                ...
                findAndReplace(childCode, "blockIdx.x", "k");
...
class T2ASTConsumer : public ASTConsumer {
    public:
        T2ASTConsumer(Rewriter *rewriter, map <const FunctionDecl *, int> *decls) : FPrinter(rewriter, decls), APrinter(rewriter, decls) {
            Finder.addMatcher(
                functionDecl(
                    hasDescendant( 
                        cudaKernelCallExpr(
...
class T2FrontendAction : public ASTFrontendAction {
    public:
        T2FrontendAction() {}
        void EndSourceFileAction() override {
            RewriteBuffer &temp = rewriter.getEditBuffer(rewriter.getSourceMgr().getMainFileID());
            temp.InsertText(0, "#include \"freeLaunch_T2.h\"\n");
        ...
        std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI,
            ...
            return make_unique<T2ASTConsumer>(&rewriter, &decls);
        }
...
int main(int argc, const char **argv) {
    ...
    return Tool.run(newFrontendActionFactory<T2FrontendAction>().get());
}
